      ...
      if (!portfolioData || portfolioData.length === 0) {
        ...
        filteredData.forEach((portfolio) => {
          ...
          row.innerHTML = `
                        ...
                        <td>${
                          portfolio.propertyOwnerName || 'Belirtilmemiş'
                        }</td>
                        <td><a href='${
                          portfolio.titleDeeds[0].files[0]
                        }' target="_elif">Dosyayı Görüntüle</a></td>
                        <td>${portfolio.isActive ? 'Aktif' : 'Pasif'}</td>
                        <td>
                            <button class="btn btn-danger btn-sm delete-btn" title="Sil" data-id="${
                              portfolio._id
                            }">
                                <i class="fas fa-trash"></i>
                            </button>
                             <button class="btn btn-success btn-sm edit-btn" title="Düzenle" data-id="${
                               portfolio._id

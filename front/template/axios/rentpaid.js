...
$(document).ready(function () {
  ...
  const table = $('#rentpaidTable').DataTable({
    ...
    initComplete: function () {
      ...
      $(this.api().table().container())
        .find('.top')
        .append(
          '<button type="button" class="btn btn-primary addPaymentModal" data-bs-toggle="modal" data-bs-target="#addPaymentModal" style="margin-left: 10px;">Ödeme Ekle</button>',
        )
    },
  })
      ...
      rentData.forEach((payment) => {
        table.row
          .add([
            ...
            payment.receipt
              ? `<a href="${fileURL}/public/${payment.receipt}" target="_blank" class="btn btn-info btn-sm">
  ...
  $('#addPaymentForm').on('submit', async function (e) {
    ...
    try {
      ...
      console.error(
        'Dosya yükleme ve ödeme güncelleme işlemi sırasında bir hata oluştu:',
        error,
      )
    }
  })

...
async function populatePersonel(personelId) {
  try {
    ...
    const filteredPersonel = response.data.data.filter(
      (personel) => personel._id === personelId,
    )
...
async function fillFormWithData(id) {
  try {
    ...
    if (customer) {
      ...
      document.querySelector('#customer-phonenumber').value =
        customer.phone || ''
      document.querySelector('#customer-birthdate').value =
        customer.birthDate || ''
...
async function populateDistricts(stateCode) {
  try {
    ...
    console.error(
      'İlçe ve mahalle seçenekleri doldurulurken hata oluştu:',
      error,
    )
  }
}

...
document.addEventListener('DOMContentLoaded', async function () {
      ...
      setTimeout(() => {
        ...
        if (container) {
          ...
          if (activeStep) {
            ...
            var stepMargin = parseInt(
              window.getComputedStyle(activeStep).marginRight,
              10,
            )
            var scrollPosition =
              (stepWidth +
                stepMargin +
                document.querySelector('.line').offsetWidth) *
              index
  ...
  async function populateForm(id) {
    try {
      ...
      const districtSelectElement2 =
        document.getElementById('titledeeddistrict')
      const neighborhoodSelectElement2 = document.getElementById(
        'titledeedneighborhood',
      )
      ...
      data_state2[property.titledeed.titledeeddistrict] &&
        data_state2[property.titledeed.titledeeddistrict].forEach(
      ...
      setInputValue(
        '#realEstateInvestmentsPortfolio',
        details.realEstateInvestmentsPortfolio,
      )
      ...
      setInputValue(
        ...
        details.propertyTransactionStatus,
      )
      ...
      setInputValue(
        ...
        titledeed.independentSectionDescription,
      )
      ...
      setInputValue(
        ...
        titledeed.titleDeedTransferMethod,
      )
      ...
      setInputValue(
        ...
        titledeed.mainPropertyDescription,
      )
  ...
  async function updateProperty() {
    const propertyData = {
      ...
      details: {
        ...
        valuationPricePerM2: document.querySelector('#valuationPricePerM2')
          .value,
        ...
        forownerpurchaseDate: document.querySelector('#forownerpurchaseDate')
          .value,

        nonRentStatus: document.querySelector('#nonRentStatus').value,
        mortgageStatus: document.querySelector('#mortgageStatus').value,
        propertyTransactionStatus: document.querySelector(
          '#propertyTransactionStatus',
        ).value,

        realEstateInvestmentsPortfolio: document.querySelector(
          '#realEstateInvestmentsPortfolio',
        ).value,
        ...
        valuationReportDate: document.querySelector('#valuationReportDate')
          .value,
        valueInValuationReport: document.querySelector(
          '#valueInValuationReport',
        ).value,
        portfolioValue: document.querySelector('#portfolioValue').value,
      },
      asset: [],
      otherDetails: {
        facade: Array.from(
          document.querySelectorAll('input[name="facade"]:checked'),
        ).map((cb) => cb.value),
        general: Array.from(
          document.querySelectorAll('input[name="general"]:checked'),
        ).map((cb) => cb.value),
        environment: Array.from(
          document.querySelectorAll('input[name="environment"]:checked'),
        ).map((cb) => cb.value),
        disabledFriendly: Array.from(
          document.querySelectorAll('input[name="disabledFriendly"]:checked'),
        ).map((cb) => cb.value),
        external: Array.from(
          document.querySelectorAll('input[name="external"]:checked'),
        ).map((cb) => cb.value),
        internal: Array.from(
          document.querySelectorAll('input[name="internal"]:checked'),
        ).map((cb) => cb.value),
        transportation: Array.from(
          document.querySelectorAll('input[name="transportation"]:checked'),
        ).map((cb) => cb.value),
        view: Array.from(
          document.querySelectorAll('input[name="view"]:checked'),
        ).map((cb) => cb.value),
        residentialType: Array.from(
          document.querySelectorAll('input[name="residentialType"]:checked'),
        ).map((cb) => cb.value),
        infrastructure: Array.from(
          document.querySelectorAll('input[name="infrastructure"]:checked'),
        ).map((cb) => cb.value),
        location: Array.from(
      ...
      titledeed: {
        ...
        independentSectionDescription: document.querySelector(
          '#independentSectionDescription',
        ).value,
        ...
        titleDeedTransferMethod: document.querySelector(
          '#titleDeedTransferMethod',
        ).value,
        titleDeedTransferDate: document.querySelector('#titleDeedTransferDate')
          .value,
        ...
        titledeedneighborhood: document.querySelector('#titledeedneighborhood')
          .value,
        ownership: document.querySelector('#ownership').value,

        restrictionStatus: document.querySelector('#restrictionStatus').value,
        mainPropertyDescription: document.querySelector(
          '#mainPropertyDescription',
        ).value,
    ...
    try {
      const response = await axios.post(
        `http://localhost:3001/api/v1/emlakze/admin/updateproperty`,
        propertyData,
        {
          headers: { 'Content-Type': 'application/json' },
        },

...
$(document).ready(function () {
        ...
        if (selectedPortfolioId) {
          ...
          if (selectedPortfolio) {
            portfolioselectElement.append(
              `<option value="${selectedPortfolio._id}" selected>${
                selectedPortfolio.portfolioName || 'Bilinmiyor'
              }</option>`,
            )
          }
        }
          ...
          if (portfolio._id !== selectedPortfolioId) {
            portfolioselectElement.append(
              `<option value="${portfolio._id}">${
                portfolio.portfolioName || 'Bilinmiyor'
              }</option>`,
            )
          }
        })
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        if (selectedPropertyId) {
          ...
          if (selectedProperty && selectedProperty.details.length > 0) {
            selectElement.append(
              `<option value="${selectedProperty._id}" selected>${
                selectedProperty.details[0].propertyName || 'Bilinmiyor'
              }</option>`,
            )
          }
        }
        ...
        response.data
          .filter(
            (property) =>
              property._id !== selectedPropertyId && property.isActive,
          )
          .forEach((property) => {
            if (property.details.length > 0) {
              selectElement.append(
                `<option value="${property._id}">${
                  property.details[0].propertyName || 'Bilinmiyor'
                }</option>`,
              )
            }
          })
  ...
  async function populateForm(data) {
    ...
    document.getElementById('startingRentAmount').value =
      rent.startingRentAmount || ''
    ...
    document.getElementById('rentIncreaseType').value =
      rent.rentIncreaseType || ''
    ...
    document.getElementById('maintenanceStartDate').value =
      rent.maintenanceStartDate || ''
    document.getElementById('maintenanceFeeAmount').value =
      rent.maintenanceFeeAmount || ''
  ...
  async function submitFormData() {
    ...
    const rentData = {
      ...
      maintenanceStartDate: document.getElementById('maintenanceStartDate')
        .value,
      maintenanceFeeAmount: document.getElementById('maintenanceFeeAmount')
        .value,
      // payments: [{
